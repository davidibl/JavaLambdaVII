...
public class Field {
	...
	public static Function<Predicate<Field>, Field> killField(Field field) {
		return (shouldDiePredicate) -> (shouldDiePredicate.test(field)) ? returnDeadField(field) : field;
	}

	public static Function<Predicate<Field>, Field> lifeField(Field field) {
		return (shouldLivePredicate) -> (shouldLivePredicate.test(field)) ? returnAliveField(field) : field;
	}
